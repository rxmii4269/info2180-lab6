...
$query  = ucwords(filter_var(htmlspecialchars($_GET["query"]), FILTER_SANITIZE_STRING, FILTER_FLAG_NO_ENCODE_QUOTES));
if ($query === '') : ?>
    <h2 class="resultHeader bold">Result</h2>
    <hr>
    <ul>
        <?php foreach ($superheroes as $superhero) : ?>
            <li class="bold"><?= $superhero['alias']; ?></li>
        <?php endforeach; ?>
    </ul>
<?php else :   ?>
    <?php $lst = 1; ?>
    <?php foreach ($superheroes as $superhero) : ?>
        <?php if ($superhero['name'] === $query || $superhero['alias'] === $query) : ?>
            <h2 class="resultHeader bold">Result</h2>
            <hr class="grey400">
            <h3 class="text-uppercase no-margin"><?= $superhero['alias']; ?></h3>
            <h4 class="text-uppercase no-margin font-size13">A.K.A <?= $superhero['name']; ?></h4>
            <p class="font-size14"><?= $superhero['biography']; ?></p>
            <?php break; ?>
        ...
        <?php else :  ?>
            ...
            <?php if ($lst === 11) : ?>
                <h2 class="resultHeader bold">Result</h2>
                <hr>
                <h4 class="red text-uppercase bold">SUPERHERO NOT FOUND</h3>
                <?php endif; ?>
